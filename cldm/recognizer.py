...
import time
from ocr_reg.RecModel import RecModel
import torch
...
def crop_image(src_img, mask):
    ...
    # import random
    # cv2.imwrite(f"result/{random.randint(0,100)}.jpg", result.permute(1,2,0).numpy())
    return result
...
def create_predictor(model_dir=None, model_lang='ch', is_onnx=False):
    ...
    if is_onnx:
        ...
        if model_file_path is not None:
            rec_model.load_state_dict(torch.load(model_file_path, map_location="cuda"))
            rec_model.eval()
