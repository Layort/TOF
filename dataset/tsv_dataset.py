...
import torchvision
from .base_dataset import BaseDataset, check_filenames_in_zipdata, recalculate_box_and_verify_if_valid , recalculate_box_and_verify_if_valid2 
...
def decode_item(item):
    ...
    item['mask'] = decode_base64_to_pillow(item['mask'])
...
class TSVDatasetNew(BaseDataset):
    def __init__(self, 
                tsv_path,
                which_layer_text='before', 
                which_layer_image="after_reproject",
        ...
        self.which_layer_image = which_layer_image
        self.prob_use_caption = prob_use_caption
        # print('prob_use_caption',prob_use_caption)
        # pdb.set_trace()
        self.random_drop_embedding = random_drop_embedding
        self.min_box_size = min_box_size
        self.max_boxes_per_data = max_boxes_per_data
        self.max_text_boxes_per_data = max_text_boxes_per_data
        self.max_images = max_images
    ...
    def __getitem__(self, index):
        ...
        # -------------------- id and image ------------------- # 
        out['id'] = raw_item['data_id']
        out['file_name'] = raw_item['file_name']

        image = raw_item['image']
         
        image_tensor  = self.transform_image(image)

        out["image"],trans_info = image_tensor
        
        # ---------------------mask of text in image --------------------------#
        text_mask_image = raw_item["mask"]
        text_mask_image_tensor,trans_mask_info = self.transform_mask_image(text_mask_image)
        out['text_mask_image_tensor'] = text_mask_image_tensor
        ...
        #---------------------text token---------------------------#
        text_annos = raw_item["text_anno"]
        all_text_boxes = []
        all_text_bboxes = []
        for text_anno in text_annos:
            wordBB = np.array(text_anno["wordBB"])
            # print("wordBB.shape",wordBB.shape)
            if(len(wordBB.shape) == 2):
                BB = wordBB
                x, y = min(BB[0,:]), min(BB[1,:])
                x1, y1 = max(BB[0,:]), max(BB[1,:])
                w, h = (x1 - x, y1 - y)
                valid, bb = recalculate_box_and_verify_if_valid2(BB, trans_info, self.image_size, 0)
                _, (x0, y0, x1, y1) = recalculate_box_and_verify_if_valid(x, y, w, h, trans_info, self.image_size, 0)
                if(valid):
                    all_text_boxes.append(torch.tensor([x0,y0,x1,y1]) / self.image_size)
                    all_text_bboxes.append(torch.tensor(bb) / self.image_size)
            else:
                for i in range(wordBB.shape[2]):
                    ...
                    if(valid):
                        all_text_boxes.append(torch.tensor([x0,y0,x1,y1]) / self.image_size)
                        all_text_bboxes.append(torch.tensor(bb) / self.image_size)
        text_boxes = torch.zeros(self.max_text_boxes_per_data, 4)
        text_bboxes = torch.zeros(self.max_text_boxes_per_data, 2,4)
        text_boexes_masks = torch.zeros(self.max_text_boxes_per_data)
        for i in range(min(len(all_text_boxes), self.max_text_boxes_per_data)):
            text_boxes[i] = all_text_boxes[i]
            text_bboxes[i] = all_text_bboxes[i]
            text_boexes_masks[i] = 1
        out["text_boxes"] = text_boxes
        out["text_bboxes"] = text_bboxes
        out["text_boexes_masks"] = text_boexes_masks
...
class TSVDatasetNew2(BaseDataset):
    def __init__(self, 
                tsv_path,
                which_layer_text='before', 
                which_layer_image="after_reproject",
    ...
    def __getitem__(self, index):
        ...
        for text_anno in text_annos:
            wordBB = np.array(text_anno["wordBB"])
            texts = text_anno["text"]
            texts = texts.split()
            sum_ = wordBB.sum()
            if(sum_ in wordBB_dict):
                wordBB =wordBB[:,:, wordBB_dict[sum_]:]
                wordBB_dict[sum_] += len(texts)
            else:
                wordBB_dict[sum_] = len(texts)
            ...
            for i in range(len(texts)):
                ...
                if(valid):
                    ...
                    all_text_boxes.append(torch.tensor([x0,y0,x1,y1]) / self.image_size)
                    all_text_bboxes.append(torch.tensor(bb) / self.image_size)
                    position = draw_pos(bb.astype(np.int32), 1.0)
                    position = (position*255.0).astype(np.uint8)
                    all_text_box_pos.append(torch.tensor(position))
                    n_lines +=1
        text_boxes = torch.zeros(self.max_text_boxes_per_data, 4)
        text_bboxes = torch.zeros(self.max_text_boxes_per_data, 2,4)
        text_boexes_masks = torch.zeros(self.max_text_boxes_per_data)
        text_box_pos = torch.zeros((self.max_text_boxes_per_data,self.image_size,self.image_size,1))
        for i in range(min(len(all_text_boxes), self.max_text_boxes_per_data)):
            text_boxes[i] = all_text_boxes[i]
            text_bboxes[i] = all_text_bboxes[i]
            text_box_pos[i] = all_text_box_pos[i]
            text_boexes_masks[i] = 1
        out["texts"]  =all_text
        out["text_boxes"] = text_boxes
        out["text_bboxes"] = text_bboxes
        out["text_boexes_masks"] = text_boexes_masks
        out["position"] = text_box_pos
        out["n_lines"] = min(n_lines,8)
