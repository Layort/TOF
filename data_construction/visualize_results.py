...
def viz_textbb(text_im, charBB_list, wordBB, k, alpha=1.0):
    """
    ...
    name = k.split('.')[0]
    save_path = os.path.join('visual_result',f'{name}.jpg')
    print(save_path)
...
def main(db_fname):
    ...
    for k in dsets:
        ...
        img = rgb[:,:,:3]
        masks = rgb[:,:,3]
        plt.imsave('viz.png', img)
        print('name',k)
        viz_textbb(img, [charBB], wordBB, k)
