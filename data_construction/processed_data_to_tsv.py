...
def item_to_encodable(item):
    ...
    item['mask'] = encode_pillow_to_base64(item['mask'])
    item['text_anno']
...
class GroundingDataset(Base):
    ...
    def __init__(self, image_root, json_path, annotation_embedding_path,mask_root):
        ...
        self.data_id_to_text_anno = defaultdict(list)
        for text_anno in self.text_anno:
            self.data_id_to_text_anno[ text_anno['data_id']].append(text_anno)
    ...
    def __getitem__(self, index):
        ...
        mask = self.fetch_mask_and_invert(file_name)
        out["mask"] = mask
        out["image"] = image
        ...
        text_anno = deepcopy(self.data_id_to_text_anno[data_id])
        for anno in annos:
        ...
        out["annos"] = annos
        out["text_anno"] = text_anno
        return out

    def __len__(self):
        return len(self.data_id_list)
    
    def fetch_mask_and_invert(self,file_name):
        image = Image.open(  os.path.join(self.mask_root,file_name)   ).convert('RGB')
...
if __name__ == "__main__":
    ...
    if args.which_dataset == "grounding":
        anno_dataset = GroundingDataset(image_root,json_path, annotation_embedding_path,mask_root)
    else:
        anno_dataset = CDDataset(image_root,json_path, annotation_embedding_path)
    ...
    anno_writer = TSVWriter(tsv_path)
    for i in tqdm(indices):
        anno_item = anno_dataset[i]
        anno_item = item_to_encodable(anno_item) # annotion信息
        row = [anno_item['data_id'], json.dumps(anno_item)]
        anno_writer.write(row)
    anno_writer.close()
