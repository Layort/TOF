...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.min_nchar = 2
        self.min_font_h = 18 #px : 0.6*12 ~ 7px <= actual minimum height
    ...
    def render_sample(self, font, mask):
        ...
        # print('H',H)
        # print('W',W)
        f_asp = self.font_state.get_aspect_ratio(font) # 返回长宽比
        # print('f_asp', f_asp)
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            # print('font',font)
            # print('text',text)
            txt_arr,txt,bb = self.render_curved(font, text) # 渲染文本 使文本扭曲
            # print('txt',txt)
...
class FontState(object):
    ...
    """
    size = [60, 6]  # normal dist mean, std  #[50,10]
    # underline = 0.05
    underline = -1
    strong = 0.2 # 0.5
    oblique = 0.15
    ...
    kerning = [2, 5, 2, 20]  # beta distribution alpha, beta, offset, range (mean is a/(a+b))
    border = 0.1 # 0.25
    ...
    def sample(self):
        ...
        """
        return {
            'font': self.fonts[int(np.random.randint(0, len(self.fonts)))], # 0,1,2
...
class TextSource(object):
    ...
    def sample_para(self,nline_max,nchar_max): # 从txt文件中随机提取多行文本内容
