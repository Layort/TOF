...
import os
from color_map.rectify_data import *
DEBUG = False
...
class TextRegions(object):
    ...
    minWidth = 0.02  # px
    minHeight = 0.02  # px
    minAspect = 0.08  # w > 0.3*h
    maxAspect = 10
    minArea =  0.05 # number of pix / all_areas
    pArea = 0.06  # area_obj/area_minrect >= 0.6

    # RANSAC planar fitting params:
    dist_thresh = 0.10  # m
    num_inlier = 90
    ransac_fit_trials = 100
    min_z_projection = 0.15 #0.25
    ...
    @staticmethod
    def filter(seg, area, label, imname):
        """
        Apply the filter.
        ...
        # print('area:',area)
        # print('label',label)
        img_width, img_height = seg.shape
        img_area = img_height * img_width

        good = label[area > img_area * TextRegions.minArea] # area
        
        area = area[area > img_area * TextRegions.minArea]
        filt, R = [], []
        # print('seg.shape',seg.shape)
        #add draw_background
        seg_rgb = Image.fromarray((seg * 255).round().astype(np.uint8), mode='L')
        seg_rgb  =  seg_rgb.resize(seg.shape,Image.NEAREST)
        draw_background = ImageDraw.Draw(seg_rgb)
        ...
        for idx, i in enumerate(good):
            ...
            rect = cv2.minAreaRect(coords) # 计算坐标的最小外接矩形
            ...
            # print('w',w)
            # print('rot',rot)
            f = (h > img_height * TextRegions.minHeight # height
                 and w > img_width * TextRegions.minWidth # width
                 and TextRegions.minAspect < w / h < TextRegions.maxAspect # maxAspect
                 and area[idx] / w * h > TextRegions.pArea) # pArea
            filt.append(f)
            R.append(rot)
        ...
        try:  
            ...
            #     box = np.array(cv2.boxPoints(rect))
            #     # print('box',box)
            #     draw_background.polygon((box),outline=128, fill=None,width=5)
            # segname = imname.split('.')[0]
            # segmentation = 'segmentation'
            # if segmentation:
            # # create a directory if not exists for masks
            #     if not os.path.exists(segmentation):
            #         os.makedirs(segmentation)
            # seg_rgb.save(f'{segmentation}'+'/segmentation_'+f'{segname}'+'.png')
            R = [R[i] for i in aidx]
            filter_info = {'label': good, 'rot': R, 'area': area[aidx]}
        except:
            print('filter is null ')
            filter_info = {'label': [], 'rot': [], 'area': []}
    ...
    @staticmethod
    def get_regions(xyz, seg, area, label,imname):
        regions = TextRegions.filter(seg, area, label, imname)
...
def get_text_placement_mask(xyz, mask, plane, imname,idx, pad=2, viz=False):
    """
    ...
    # 文本绘制区可视化
    # visuation_path = 'visualization_14'
    # if visuation_path:
    # # create a directory if not exists for masks
    #     if not os.path.exists(visuation_path):
    #         os.makedirs(visuation_path)
    # # 绘制 mask
    # axes[0].imshow(mask, cmap='gray')
    # axes[0].set_title("Original Mask")
    
    # # 绘制放置掩膜
    # axes[1].imshow(~place_mask, cmap='gray')
    # axes[1].set_title("Placement Mask")

    # # 标记轮廓点
    # for i in range(len(pts_fp_i32)):
    #     axes[1].scatter(pts_fp_i32[i][:, 0], pts_fp_i32[i][:, 1],
    #                     edgecolors='none', facecolor='g', alpha=0.5)
    
    # viname = imname.split('.')[0]
    # # 保存图像
    # plt.savefig(f'{visuation_path}'+"/visualization_"+f'{viname}'+'_'+f'{idx}'+".png")
    # plt.close(fig)  # 关闭绘图对象
    ...
    if viz:
        ...
        for i in range(len(pts_fp_i32)):
            plt.scatter(pts_fp_i32[i][:, 0], pts_fp_i32[i][:, 1],
                        edgecolors='none', facecolor='g', alpha=0.5)
        plt.show()
    # print('place_mask', place_mask.shape)
    # print('H', H.shape)
...
class RendererV3(object):
    ...
    def __init__(self, data_dir, max_time=None):
        self.text_renderer = tu.RenderFont(data_dir) # text content
        self.colorizer = Colorize(data_dir) # font color
        self.min_char_height = 12  # px 
        self.min_asp_ratio = 0.4  # guan gao bi >big
        self.max_text_regions = 6 # 最多的文本区域
    ...
    def filter_for_placement(self, xyz, seg, regions, imname):
        filt = np.zeros(len(regions['label'])).astype('bool')
        masks, Hs, Hinvs = [], [], []
        for idx, l in enumerate(regions['label']):
            res = get_text_placement_mask(xyz, seg == l, regions['coeff'][idx], imname, idx, pad=2)
            # print('res', res.shape)
            if res is not None:
                mask, H, Hinv = res
                masks.append(mask)
    ...
    def place_text(self, rgb, collision_mask, H, Hinv):
        ...
        # feathering: #是否要加入羽化效果
        # text_mask = self.feather(text_mask, min_h)
        
        im_final, fg_col, bg_col  = self.colorizer.color(rgb, [text_mask], np.array([min_h]))

        return im_final, text, bb, text_mask, fg_col, bg_col 
    ...
    def render_text(self, rgb, depth, seg, area, label, imname, ninstance=1):
        """
        ...
        try:
            ...
            # find text-regions:  
            regions = TextRegions.get_regions(xyz, seg, area, label, imname)
            # print('region 1\n')
            # print(regions)
            # find the placement mask and homographies:
            regions = self.filter_for_placement(xyz, seg, regions, imname)
            # print('region 2\n')
            # print(regions)

            # finally place some text:
            nregions = len(regions['place_mask'])
            ##### the number of regions that satisfies the requirements
        ...
        for i in range(ninstance):
            ...
            # process regions: 
            num_txt_regions = len(reg_idx) #每次从所有文本区域中随机生成几个区域（可能是全部区域）
            NUM_REP = 3  # re-use each region three times: # 在每个文本块，渲染三次
            ...
            flag  = [False]*m
            for idx in reg_range: #每次对随机生成文本区域中，在每一个区域渲染最多渲染一次文本内容
                ireg = reg_idx[idx]
                # print('ireg',ireg)
                try:
                    if flag[ireg]==False:
                ...
                if txt_render_res is not None and flag[ireg]==False :
                    if DEBUG:
                        print("    ... text rendering attempt finished successfully")
                        pass
                    placed = True
                    flag[ireg] = True
                    img, text, bb, collision_mask, fg_col, bg_col  = txt_render_res
