...
from  render_images import render_text_image_custom
...
def run(meta, config, starting_noise=None):
    ...
    print('metahint',type(meta["hint"]))

    hint = np.array([meta["hint"]] * config.batch_size)  
    ...
    print('samples_fake.shape',samples_fake.shape)
...
def get_hint(image_resolution, rendered_txt_values,  width_values, ratio_values, top_left_x_values, top_left_y_values, yaw_values, num_rows_values ):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder", type=str,  default="generation_samples_text_object", help="root folder for output")
    ...
    parser.add_argument("--glyph_instructions", type=str, default="glyph_instructions.yaml", help="path to glyph instructions",)  
    ...
    hint = get_hint(args.image_resolution,rendered_txt_values, width_values, ratio_values, top_left_x_values, top_left_y_values, yaw_values, num_rows_values)
    arr = np.array(hint)
    arr = 1 - arr.astype(np.float32) / 255.0
    arr = np.transpose(arr, [2,0,1])

     # - - - - - - - - text_object layout for generation - - - - - - - - # 
    dict(
            ckpt = "./text_object_systhesis.pth",
            prompt = "Two artistic mugs",
            phrases = ['mug','mug'],
            locations = [ [0.1,0.1,0.5,0.9], [0.5,0.1,0.9,0.9] ],
            alpha_type = [0.3, 0.0, 0.7],
            save_folder_name="generation_mugs",
            hint = arr
    ), 

    dict(
            ckpt = "./text_object_systhesis.pth",
            prompt = "a T-shirt on the left, a pair of pants on the right",
            phrases = ['T-shirt','pants'],
            locations = [[0.05,0.25,0.5,0.9],[0.55,0.28,0.9,0.9]],
            alpha_type = [0.3, 0.0, 0.7],
            save_folder_name="generation_clothes",
            hint = arr
    ), 
